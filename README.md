...
| SETI drift-search benchmark | `jansky_research.driftsearch` | ✅/➖ benchmark built; the DC-spike artifact story stands, but the round-11 referee found the "Voyager null" was a targeting error — the detector recovers the real carrier (8419.297 MHz) at S/N ≈ 998 |
...
| A SETI drift-search benchmark + Voyager-1 null | `driftsearch/` | benchmark + negative result | 100% | **major revision** ([referee 2026-08-26](survey/drift-findings.md)) | 🔄 in revision — the "null" searched blank sky: the hand-typed 8420.216 MHz maps to an empty channel, while the actual carrier (8419.29703 MHz, ±22.5 kHz telemetry sidebands, −0.374 Hz/s — not the hand-typed −0.69) is recovered by the module's own search at S/N = 997.6; no committed Voyager evidence, no DC-spike regression test, benchmark config unstated |
...
| A streaming e-Callisto burst-ingest pipeline with cross-station coincidence QC | `ecallisto_pipeline/` | automation pattern + coincidence-vetted burst events (rejects single-station RFI) | — | **major revision** ([referee 2026-08-26](survey/ecallisto-findings.md)) | 🔄 in revision — the committed figure is the real leg's output under the synthetic caption; n_scanned=1512 double-counts every file (763 listed, 559 fetched); 64% of rows analysed a DIFFERENT file than labelled (corrupting the 60 s coincidence clock); the "8 candidates" are 2 detections ×4 duplicates; no FP rate; the real-data null (0 events ever confirmed) buried |
...
| RM structure functions from SPICE-RACS DR1 | `rmstructure/` | method + recover-a-known + bounded high-\|b\| estimate | 96% | **major revision** ([referee 2026-08-26](survey/rmstructure-findings.md)) | 🔄 in revision — the quality-flag break claim (2.29°→0.87°) rests on an 11-valued uncommitted statistic on a visibly noise-dominated curve; the ladder's six error bars use the bootstrap the paper condemns as 11× too small; the CI window passes only via the outlier seed (17/30 committed seeds would fail it); block size never varied; the figure mislabels its axis and saturates on the headline contrast |
...
| How often are long-period transients on? (VAST archival; RNAAS note) | `lptduty/` | sensitivity-weighted duty-cycle limits for the LPT class | 96% | **major revision** ([referee 2026-08-26](survey/lptduty-findings.md)) | 🔄 in revision, SUBMISSION HELD — "removes roughly half the exposure" is false by ~50× (the weighting is inert at the quoted fluxes); \ldNNoEpoch/\ldNPhaseUsable contradicted by their own JSON; a Kuiper statistic quoted with the Rayleigh p; Ṗ smear makes 3 more constraints conditional; p is Stokes-V presented as pulse probability; dividing p by each source's own (w+T)/P inverts the story (J183950 implies f_active ≈ 0.73) |

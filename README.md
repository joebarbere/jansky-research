...
|-------|-----------|---------|-------|------|----------|
| FRB burst statistics, validated on CHIME/FRB Cat 1 | `frbstats/` | validation (tested, reproducible tool) | 97% | **major revision** ([referee 2026-08-25](survey/findings.md)) | 🔄 in revision — Figure 1 plots pooled waits under a within-source caption (a good fit shown as a bad one); 62 bursts from 18 sources treated as 62 draws (DM's 1e-11 is honestly 0.044; width survives at 1e-5); the macro writer has no clobber guard |
...
| Image-plane stacking with injection-recovery: SDSS quasars in VLASS-SE | `stacking/` | methodology + calibrated population-mean flux | 99% | **major revision** ([referee 2026-08-25](survey/stacking-findings.md)) | 🔄 in revision — the injection-recovery ratio is an algebraic identity (≡1.0 for any input), and every flux/SNR is a searched peak biased +1.6σ on blank sky (headline 4.5σ ≈ 3.6σ honest; the binned-vs-full 1.42× discrepancy in the committed JSON is the fingerprint) |
| Multi-decade parsec-scale VLBI variability from Astrogeo | `vlbi/` | control-floor method + recover-a-known (OJ 287, BL Lac) | 100% | **major revision** ([referee 2026-08-25](survey/vlbi-findings.md)) | 🔄 in revision — the load-bearing calibrator citation is by Fassnacht & Taylor (Peck was never an author); the median floor admits 50% of steady controls by construction (headline 13/14 runs 13→10 across the control span, with one tie at the committed precision); V confounded with epoch count at p = 0.004 |

...
| DR20 BHM radio-counterpart census (plan 88) | `jansky_research.dr20radio` | ✅ first radio cross-match of any SDSS-V BHM quasar catalog, four days after the DR20 release: north 202,691 quasars × VLASS → 4.67% any-epoch (measured chance rate 0.009%); **the categorical first** — 73,074 quasars south of δ=−40° (the first southern SDSS spectra, all LCO) × RACS-low → **3.95%**; the cross-frequency spectral index **measured, not assumed** — 4,190 quasars detected by both VLASS and RACS in the survey-overlap band above the joint-detection completeness flux give α = −0.72 ± 0.015 (stat) ± 0.36 (sys), an upper bound on flatness (the median steepens monotonically as the assumed completeness floor is lowered); at that index the north/south gap is 1.27 pp, falling 28% to 0.91 pp once the two surveys' *definitionally different* flux limits are equalised, and no astrophysical inference is drawn; radio-targeted cartons excluded as circular and repurposed as a cross-frequency recovery bound (89% selecting survey → 49%/27%/17%, consistent within their binomial errors) |
...
| DR20 BHM radio-counterpart census | `dr20radio/` | first radio census of the first southern SDSS quasar spectra (RACS 3.95%, VLASS north 4.67%) + a north/south contrast reported as an **α-dependent range** (0.23–1.66 pp), with measured chance rates, a measured RACS-footprint bound, and carton cross-frequency controls | 99% | **major revision** ([3 referee rounds](survey/dr20radio-findings.md)) | ✅ 2026-08-12 — 37 findings over 3 rounds; α now measured |

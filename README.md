...
| The AGN radio–optical offset excess and its alignment with the jet (ICRF3 × Gaia × MOJAVE) | `offsets/` | reproduction (excess + jet alignment) + reproducible catalogue | 99% | **minor revision** ([referee 2026-08-25](survey/offsets-findings.md)) | 🔄 in revision — every number reproduces exactly, but the Rayleigh null ignores the RA/Dec correlation ICRF3 ships (24.0× → ≈21×); the directional test uses a null the axis result already excluded; the promised matched catalogue is not shipped |
...
| 3D triangulation of a type III source with STEREO-A+B direction-finding | `triangulate/` | method + independent geometric-vs-plasma distance cross-check | 97% | **major revision** ([referee 2026-08-25](survey/triangulate-findings.md)) | 🔄 in revision — the published longitude is a scalar median across the ±180° wrap (168.9° vs wrap-safe ~179°); the "factor 2.18" is really a constant additive ~14 R⊙ offset (a ~3.7° DF bias), leaving no room for the density-enhancement narrative |
| Recovering the canonical 1.4 GHz Euclidean source counts from NVSS | `sourcecounts/` | reproduction (Hopkins 2003) | 100% | **major revision** ([referee 2026-08-25](survey/sourcecounts-findings.md)) | 🔄 in revision — every number reproduces, but 1.021 is really 1.02 ± 0.03 (binning set by the single brightest source), the 3.5 mJy cut does not clear the completeness roll-off, the sample is components vs a FIRST-anchored reference, and the fixture cannot fail |
| A type III beam from the corona to 0.4 AU, geometrically validated | `type3synthesis/` | synthesis + same-event geometric check on the model distance | 99% | **major revision** ([referee 2026-08-25](survey/type3synthesis-findings.md)) | 🔄 in revision — five per-leg values are pre-revision sibling vintages (one is an estimator its own sibling rejected), and the "shape agreement, 2.18× scale offset" claim is inverted by its own committed channels (the offset is additive, ~13 R⊙) |

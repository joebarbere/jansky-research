...
| SETI drift-search benchmark | `jansky_research.driftsearch` | ✅ benchmark + a real recovery: the carrier is located in the data (8419.297 MHz, −0.374 Hz/s) and recovered at S/N ≈ 997; the DC-spike caution stands (artifact reads 2×10⁵); the earlier "Voyager null" retracted as a targeting error |
| HI rotation curve | `jansky_research.hi` | ✅ the inner Milky Way curve from 71 LAB sightlines, cross-validated against the VGPS (edge fit agrees to 1.04 ± 0.15 km/s in level and matches its slope); non-Keplerian by ~90 km/s, but **not flat** — the rise is 3.1σ, retracting the earlier 8-point "consistent with flat" |
...
| Cassini SKR proximity census (RPWS KEY60S) | `jansky_research.skr` | ✅/➖ the flux-series anchor certifies the band-integrated series against the published dual period (10.68/10.80 h, to 0.05%); the raw 3.3× proximity trend stands, but its 1/r² decomposition is retracted as estimator-dependent (corrected ratios span 1.4–3.6), still entangled with a 28° latitude difference |
...
| The inner Milky Way rotation curve from LAB HI, validated against the VGPS | `hi/` | validation | 93% | **major revision** ([referee 2026-08-24](survey/hi-findings.md)) | ✅ 2026-08-31 — resampled 8 → 71 longitudes and compared against MG&D 2016's tabulated VGPS curve: the edge fit agrees to 1.04 ± 0.15 km/s over 49 overlapping sightlines and reproduces its slope (2.52±0.55 vs 3.82±0.18). The same sample **retracts the flatness claim** — 2.8±1.8 (1.6σ) at N=6 becomes 1.79±0.57 (3.1σ) at N=51 — and the title lost "Flat". Threshold bias confirmed at +15.26±0.56 vs the VGPS, not constant (sd 3.92), closing to +4.2 at MG&D's own 20 K |
| A SETI drift-search benchmark + Voyager-1 recovered | `driftsearch/` | benchmark + real-data recovery + DC-spike caution | 100% | **major revision** ([referee 2026-08-26](survey/drift-findings.md)) | ✅ 2026-08-26 — the carrier is LOCATED in the data (never asserted): 8419.29696 MHz, −0.3741 Hz/s measured, recovered at S/N = 997 (committed, macro'd, regression-tested incl. a gated real-file test); the null retracted in print with the 0.92 MHz lesson kept as evidence; benchmark config committed; full p_detect matrix + noise-peak distribution + off-grid check; figure axis fixed |
...
| A streaming e-Callisto burst-ingest pipeline with cross-station coincidence QC | `ecallisto_pipeline/` | automation pattern + coincidence-vetted burst events (with its chance-coincidence rate) | — | **major revision** ([referee 2026-08-26](survey/ecallisto-findings.md)) | ✅ 2026-08-26 — fetch-by-filename + de-duplicated listings (the real day: 763 listed → 761 scanned, 2 failures counted, 2 candidates — BIR's is the known 11:50 UT burst, auditable from the committed t_peak_s); per-leg figures; DAG and CLI share one scan_file; the fixture gained contaminants that can defeat it (60/60-seed ensemble committed); event spans + closed-form chance rate quoted; the real-day null promoted to the abstract with its honest reading (13 of 15 stations saw nothing: the threshold, not coverage, binds) |
...
| RM structure functions from SPICE-RACS DR1 | `rmstructure/` | method + recover-a-known + bounded high-\|b\| estimate | 96% | **major revision** ([referee 2026-08-26](survey/rmstructure-findings.md)) | ✅ 2026-08-26 — the quality-flag break claim RETRACTED: the matched-pair comparison it lacked shows the shift preferred in only 37.5% of shared bootstrap replicates (both curves + intervals committed); every ladder error now a sky-block jackknife (bootstrap hid a factor ~7); block-size sweep committed with the fixture arbitrating 10° (1.1 calibrated, 1.7 ceiling); definitional variants run on DR2 (alt bins move the ratio +54%); CI asserts on the ensemble, not the outlier seed; per-leg figures in Galactic coordinates with the ladder panel |

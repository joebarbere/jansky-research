...
| The steep radio spectra of pulsars from the ATNF catalogue | `pulsarspec/` | reproduction + MSP/normal comparison | 100% | **major revision** ([referee 2026-08-25](survey/pulsarspec-findings.md)) | 🔄 in revision — every number reproduces, but the selection is coverage (764 have S400), not the stated flux limit; the completeness cut moves −1.77 → −1.88; "offsets > 0.24 excluded" is the 50%-power threshold, not an exclusion |
...
| The Galactic Faraday rotation sky from the Taylor et al. 2009 RM catalogue | `rmsky/` | reproduction (plane enhancement + sign antisymmetry) | 98% | **major revision** ([referee 2026-08-25](survey/rmsky-findings.md)) | 🔄 in revision — every quoted error is the i.i.d. bootstrap this repo already measured understating this statistic 11× on a correlated sky; "match the literature" has no comparand; the Aitoff longitude labels are −l |
| The pulsar P–Ṗ diagram from the ATNF catalogue | `ppdot/` | reproduction + population classes | 100% | **minor revision** ([referee 2026-08-25](survey/ppdot-findings.md)) | 🔄 in revision — census reproduces line-for-line, but the "magnetar" median is set by its own threshold (12.7–14.1 across cuts), the snapshot is 2017-vintage undisclosed, and the asserted sweeps need committing |
...
| torch-dsp: the coherent-DSP suite in pure PyTorch | `torchdsp/` | per-kernel oracle validation + real CHIME/Crab legs (CPU) + ROCm benchmarks | 93% | **major revision** ([referee 2026-08-25](survey/torchdsp-findings.md)) | 🔄 in revision — the committed benchmark table is a hand-assembled splice (three inconsistent CPU sets exist) and the abstract's "10×" contradicts its own parenthetical (8.42/0.65 = 13); portability asserted, never measured |

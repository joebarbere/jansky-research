...
| RM structure functions (SPICE-RACS DR1) | `jansky_research.rmstructure` | ✅/➖ noise-debiased SF per \|b\| bin; high-\|b\| plateau is an upper bound (intrinsic-scatter-dominated); the DR2 disc–halo ladder is measured |
...
| JBO glitch waiting-time classification (F11) | `jansky_research.glitchpop` | ✅ gap-robust waiting-time classification of the live JBO catalogue (31 classifiable: 20 exponential, 10 quasi-periodic, 1 clustered); J0537 and Vela recovered as quasi-periodic, which requires excising monitoring gaps |
...
| VLASS multi-epoch variability: a 703 deg² census + FK Com | `vlass/` | methodology + validation (recovers FK Com) | 92% | **major revision** ([referee 2026-08-24](survey/vlass-findings.md)) | 🔄 in revision — the 52% completeness is an injection error-model artifact; the second candidate's rejection exists only in prose |
...
| OVRO-LWA type II burst census | `typeii/` | slow-drift detector + in-memory streaming of the whole 2024–2026 archive → a null (a blind spectral census is false-positive dominated) | 96% | **major revision** ([referee 2026-08-24](survey/typeii-findings.md)) | 🔄 in revision — arxiv.yaml still says "48 days"; the match-rate deficit reverses once the CDAW coverage gap is respected |
...
| JBO glitch waiting-time classification | `glitchpop/` | monitoring-gap-robust per-pulsar waiting-time classification of the live JBO catalogue (exp/quasi-periodic) + post-2018 change table; recover-a-known passes (J0537 & Vela quasi-periodic, gap excision required) | 95% | **major revision** ([referee 2026-08-24](survey/glitchpop-findings.md)) | 🔄 in revision — the title flip is 0.15 MC-SE from its own threshold; no committed catalogue snapshot |
| RM structure functions from SPICE-RACS DR1 | `rmstructure/` | method + recover-a-known + bounded high-\|b\| estimate | 96% | **major revision** ([referee 2026-08-24](survey/rmstructure-findings.md)) | 🔄 in revision — ±0.10 is the i.i.d. bootstrap the slice's own lesson warns about; sample definition conflicts with the release counts |

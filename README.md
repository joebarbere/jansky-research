...
| Recovering FRB 20180916B's 16.35-day period | `frbperiod/` | validation | 100% | **major revision** ([referee 2026-08-26](survey/period-findings.md)) | 🔄 in revision — 5 of the 19 "bursts" are same-transit duplicate phases (Z² 33.4 → 23.9 collapsed); the measured null is 10⁻³–10⁻², not the abstract's disclaimed-yet-printed 7×10⁻⁶ (burst clustering, not aliasing, is the culprit); the "correct null" search had 34% power against a twin of the detected source; the honest headline is the coincidence probability (~1/300), never computed |
...
| A coverage-corrected type III occurrence census (method + recover-a-known) | `ecallisto_census/` | census statistic + recover-a-known validation toward a multi-cycle census | 100% | **major revision** ([referee 2026-08-26](survey/ecallisto-census-findings.md)) | 🔄 in revision — 123 of the "168 sampled days" were failed ingests recorded as zero coverage (live-verified vs the archive; only 45 days entered); the honesty sentence is now false; the recover-a-known is circular (E[N/C]=kS by construction) and the falsifiable saturating-confirmation variant FAILS informatively; five \ecsReal* macros were typed by hand |
...
| A provenance-carrying LPT population catalogue | `lpt/` | verified table + regenerable P–Ṗ statistics (novelty scoped vs the review's own diagram) | 100% | **major revision** ([referee 2026-08-26](survey/lpt-findings.md)) | 🔄 in revision — the only figure predates two CSV corrections (one object plotted 109× below its true Ṗ limit; title says 13 objects vs abstract's 16); a catalogue paper with no catalogue table, no DAS, 10 of 16 discovery papers uncited; "exact" p is Monte Carlo (true exact 0.526); three unknown-status objects counted as non-binary move Δlog P 0.176→0.413; power ≈ size at the observed offset; the Rea+2026 accusation itself HOLDS (Crossref-corroborated) |
...
| e-Callisto megaconstellation RFI trend | `rfitrend/` | notch-robust narrowband-UEM-line metric over 2012–2026 + pipeline cross-station coherence verdict → a systematics-limited null (stations trend in opposite signs; no Starlink attribution) | 91% | **major revision** ([referee 2026-08-26](survey/rfitrend-findings.md)) | 🔄 in revision — HUMAIN's "rise" is two single-month instrumental steps (−12 units at 2018.9, +15 at 2022.2; no regime rises, and the DOWN-step spans Starlink deployment); the load-bearing reference has a fabricated author (it's Prieto, Bussons Gordo, et al.); the null has no sensitivity statement (60–70% power vs a signal the size of the data's own largest excursion); "sample-size artifact" refuted by the equal-n test |

...
def _write_macros(m: dict, path) -> None:
    ...
    lines = [
        ...
        rf"\newcommand{{\vlassArea}}{{{m['area_deg2']:.0f}}}",
        rf"\newcommand{{\vlassNsources}}{{{m['n_sources']}}}",
        rf"\newcommand{{\vlassNusable}}{{{m['n_usable']}}}",
        rf"\newcommand{{\vlassNexcluded}}{{{m['n_excluded_crowded']}}}",
        rf"\newcommand{{\vlassNcand}}{{{m['n_candidates']}}}",
        rf"\newcommand{{\vlassNconfirmed}}{{{m.get('n_image_confirmed', 0)}}}",
        rf"\newcommand{{\vlassSigma}}{{{m['sigma']:.0f}}}",
        rf"\newcommand{{\vlassComplFifty}}{{{m['completeness_50_factor']:.0f}}}",
        rf"\newcommand{{\vlassComplMax}}{{{100 * m['completeness_max']:.0f}}}",
    ]
    p = Path(path)

...
def _write_macros(m: dict, path: str | Path) -> None:
    ...
    for ns in ("tiiSyn", "tiiReal"):
        live = ns == pref
        gg = (lambda k: g(k)) if live else (lambda k: "--")
        lines += [
            rf"\newcommand{{\{ns}Source}}{{{m['source'] if live else '--'}}}",
            rf"\newcommand{{\{ns}NEvents}}{{{gg('n_events')}}}",
            rf"\newcommand{{\{ns}Completeness}}{{{gg('completeness')}}}",
            rf"\newcommand{{\{ns}Purity}}{{{gg('purity')}}}",
        ]
        ...
        for snr, tag in (
            ...
            v = curve.get(snr) if live else None
            lines.append(rf"\newcommand{{\{ns}Comp{tag}}}{{{'--' if v is None else v}}}")
    for ns in ("tiiSyn", "tiiReal"):
        live = ns == pref
        for macro, key in (

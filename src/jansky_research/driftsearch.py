...
def _write_macros(m: dict, path) -> None:
    ...
    lines = [
        ...
        rf"\newcommand{{\dsThreshold}}{{{m['threshold']:.0f}}}",
        rf"\newcommand{{\dsNtrials}}{{{m['n_trials']}}}",
        rf"\newcommand{{\dsCfifty}}{{{m['completeness_snr_50']:.1f}}}",
        rf"\newcommand{{\dsCninety}}{{{m['completeness_snr_90']:.1f}}}",
        rf"\newcommand{{\dsNdrift}}{{{len(m['drift_rates'])}}}",
        rf"\newcommand{{\dsDriftMax}}{{{max(m['drift_rates']):.1f}}}",
    ]
    p = Path(path)

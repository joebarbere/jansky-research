...
never independently re-examined. We reprocess the complete released archive
...
$d=\aiDistanceAu$~au) coincides exactly with the published one for a transmitted
linewidth equal to the channel resolution and unit dedrifting efficiency. Unit
dedrifting efficiency holds only on the drift grid; off the grid, the coarse step costs up to a
factor $\sim$3. The survivors separate into three classes of filter evasion:
sub-threshold terrestrial carriers, chance coincidences in dense hit lists, and
satellite downlinks that defeat two-position geometry by construction,
independently recovering the design rationale of BL's interference rejection. The entire reproduction
cost $\approx$95~h of active compute and $\approx$3.7~TB of transfer on a single
desktop computer.
\end{abstract}
...
$\sim$100~mW EIRP \citep{jacobsonbell2025}, with independent-observation nulls
...
independent reanalysis of the same released data with a pipeline sharing no code
...
bank-boundary frequency coverage and were assumed redundant and searched once;
byte-level identity was not verified. The recorded spans exceed
...
\aiTotNodes\ unique node-cadences, \aiTotScans\ fine-resolution scans
(366 files were transferred; one node was re-run and its first pass retained).
...
(i) ABACAD geometry: a hit must appear, extrapolated along its own drift rate,
in every on-target scan and no off-target scan.

(ii) Drift coherence: for each survivor the pipeline re-reads $\pm$2048-channel
postage stamps from all six scans and demands the tone track its fitted drift through
the full $\sim$30-min cadence, with direct off-scan cleanliness at $8\sigma$, which
subsumes the original search's asymmetric ($16\sigma$/$10\sigma$) on/off thresholds.
The L-band leg was run with a stricter symmetric $16\sigma$ on both positions; this
passes more borderline interference forward and is conservative for a null. The
S/C/X legs used the asymmetric $16\sigma$/$10\sigma$ convention. Two-position survivor
counts are therefore not directly comparable between L and S/C/X: a $10\sigma$ off-scan
veto removes candidates a $16\sigma$ veto retains.
Exactly-zero drift is flagged terrestrial.

(iii) Frequency allocation: survivors inside ITU satellite-downlink allocations
...
rejects them by geometry; this is the same rationale as BL's exclusion masks.
...
(excised). The detector is validated by single-tone round-trip on that synthetic cadence;
its recovery completeness as a function of signal-to-noise and drift rate has not
been measured for this implementation. The 3I/ATLAS
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/atlas3i_survey.pdf}
\caption{Hits per 5-min scan at $16\sigma$ for all \aiTotNodes\ unique nodes, grouped by
receiver band in order of increasing frequency. The contrast between the L-band interference
environment (up to $\sim$2$\times$10$^{5}$ hits per scan) and the C band, where individual
scans record zero hits, dominates; the full vetting funnel is \aiTotRawHits\ raw hits
$\to$ \aiTotSurvivors\ on/off survivors $\to$ \aiTotConfirmed\ confirmed.}
\label{fig:survey}
\end{figure*}
...
Separately, the C band produced zero two-position survivors across its
...
$\le\aiEirpMw$~mW at L band. Setting the transmitted linewidth of the published
formulation \citep{gajjar2021} equal to the channel resolution
($\delta\nu_t = \delta\nu$) and taking unit dedrifting efficiency $\beta$, the two
expressions coincide exactly in the combined EIRP, so the agreement with the paper's
$\sim$100~mW follows from shared parameters rather than coincidence.

This limit is quoted at $\beta=1$, which holds only for a tone landing on a drift grid
point. The $129$-point grid steps by $0.0625$~Hz\,s$^{-1}$, $6.7\times$ coarser than the
matched step $\delta\nu/\tau = 0.0093$~Hz\,s$^{-1}$; a worst-case off-grid tone smears over
$3.4$ channels in a scan, and because the shift-and-sum uses integer channel shifts the
peak-channel signal-to-noise falls roughly linearly with that smearing. The effective limit
for off-grid drift rates is therefore up to $\sim3\times$ weaker than the quoted value.
This applies at all drift rates, not only large ones, and includes the
$\sim$0.17~Hz\,s$^{-1}$ Earth-rotation drift expected for a transmitter on the target.
A matched-step search would remove the penalty at proportionally greater cost.

\subsection{Filter-evasion classes}
The \aiTotSurvivors\ two-position survivors separate into three classes.
Sub-threshold terrestrial carriers (L band, 1302--1308~MHz, aeronautical radionavigation)
passed the on/off filter by sitting below the search threshold in off scans while above
it in on scans; the stamp vet detects them directly in the off data. Chance coincidences
arise in the densest hit lists ($\sim$10$^{5}$ per scan) at the $\pm$32-channel matching
tolerance; none track their own drift. Satellite downlinks are the third
class: \aiSatCoherent\ candidates passed both geometric filters, all in the
...
5 5600X, 64~GB RAM), CPU only. Auditing a published technosignature search from its
public archive requires no institutional resources.

\section{Discussion}
Three caveats bound the claim. First, same-data reproduction validates the analysis, not the
observation: a signal absent from the recorded voltages is beyond any reanalysis.
Second, detector conventions differ: the per-drift-row MAD threshold used here fires far
more often than \code{turboSETI}'s normalization in dense interference (our L-band raw
hit count alone exceeds the original survey's full-band total), so raw hit counts are
not comparable across pipelines, and for a null this is the conservative direction,
admitting more false positives into vetting and still confirming none. Third, the
recorded band edges searched here (e.g.\ 939~MHz, 12.4~GHz) lie in receiver roll-off
the original team excluded; all survivors unique to those regions failed vetting
regardless.

With those bounds, the BL 3I/ATLAS GBT nondetection is robust to full independent
reanalysis of the released data across the complete 1--12~GHz coverage, at matching
quoted sensitivity (at L band), and the audit is cheap enough to become routine for
high-visibility technosignature results.

\software{\code{jansky-research} \citep{janskyresearch}, \code{jansky} \citep{jansky},
NumPy, h5py, astropy. Analysis assisted by Claude \citep{anthropicclaude}. The
pipeline's scientific kernels are covered by an offline test suite; the network and
real-data drivers are not unit-tested.}

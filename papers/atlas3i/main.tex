...
$d=\aiDistanceAu$~au) coincides exactly with the published one for a transmitted linewidth equal to the
channel resolution and unit dedrifting efficiency --- a condition met only on the drift
grid, off which the coarse grid step costs up to a factor $\sim$3. The survivors form a physically interpretable taxonomy of
...
bank-boundary \emph{frequency} coverage and were assumed redundant and searched once;
...
the full $\sim$30-min cadence, with direct off-scan cleanliness at $8\sigma$ ---
subsuming the original search's asymmetric ($16\sigma$/$10\sigma$) on/off thresholds.
The L-band leg was run with a stricter symmetric $16\sigma$ on both positions --- which
passes \emph{more} borderline interference forward, conservative for a null --- while the
...
its recovery completeness as a function of signal-to-noise and drift rate has \emph{not}
...
This applies at \emph{all} drift rates, not only large ones, and includes the

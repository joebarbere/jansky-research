...
\citet{degasperin2018} catalogue --- built from the same uncorrected ADR1 fluxes ---
the field shows no population offset
($+\usRealDeltaAlpha\pm\usRealDeltaAlphaSE$, \usRealNPairs\ pairs), yet the
candidates sit $\usRealFlaggedOffset$ steep: the amount a zero-free-parameter model
of selection on the noisy index predicts ($\usRealSelPred\pm\usRealSelPredSD$).
Against the reference's own USS population the cut is \usRealPurityPct\% pure and
\usRealCompletenessPct\% complete; the incompleteness is structural, since a joint
detection is equally sensitive only to $\alpha\approx\usAlphaMatch$ and flagging
$\alpha=\usAlphaProbe$ requires $S_{150}\geq\usSNeededProbe$~mJy. A cautionary
...
The follow-up scores the \emph{whole field} against the 1.4-million-source
...
earlier draft of this note told. First, the population shows \emph{no offset}:
$+\usRealDeltaAlpha\pm\usRealDeltaAlphaSE$ over \usRealNPairs\ detection pairs,
identical on both sides of the rescaled-coverage declination edge. Second, the
...
excluded) --- but treating the reference as truth and re-selecting on truth plus each
source's own noise predicts $\usRealSelPred\pm\usRealSelPredSD$: the offset is
selection on the extreme of a noisy variable, not calibration. Third, against the
...
flagged at all --- genuine USS sources are truncated out by construction.
...
offset $+\usRealDeltaAlpha$); the raw USS candidates (stars) sit below it by the
amount the committed selection model predicts. The open circle is the candidate

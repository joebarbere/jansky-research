...
SLICES ?= frbstats frbperiod driftsearch spectra hi vlass peaked southern offsets pulsarspec stacking vlbi solarbursts rmsky ppdot windwaves swaves triangulate sourcecounts type3synthesis ecallisto_pipeline ecallisto_census torchfdmt torchdsp rmstructure rmdipole frbwait frblens lpt junodam stokesv stokesv_discovery wdpulsar fashienv svsbi lptv skr typeii rfitrend vgpra pte2
...
reproduce: ## Full reproduction on REAL public data -> figures+macros -> papers -> arXiv packages
	...
	test -f data/FRB20181231C_24366209_beamformed.h5 && uv run --extra fdmt --extra voyager python -m jansky_research.torchdsp --benchmark --out . || uv run --extra fdmt python -m jansky_research.torchdsp --offline --out .  # torch-dsp (GPU: rerun with --device cuda from the ROCm venv)
	...
	uv run python -m jansky_research.fashienv --out .  # FASHI DR1 env-split HIMF (VizieR fetch; DR1 first leg, DR2 swap ~Aug 2026)
	uv run python -m jansky_research.svsbi --offline --out .  # SBI physics + prior-predictive (real NPE leg: --device cuda from the ROCm venv with the sbi extra)
	test -f results/lptv_realtargets.csv && uv run python -m jansky_research.lptv --out . || uv run python -m jansky_research.lptv --offline --out .  # LPT v3 + Stokes-V table (sweep: uv run --extra vlass python scripts/lptv_real.py)
